...
function isAuthenticated(req, res, next) {
    console.log(`[isAuthenticated] Checking session for userId: ${req.session.userId}`);
    if (req.session.userId) {
        next();
    } else {
        console.log('[isAuthenticated] User NOT authenticated. Sending 401.');
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        req.session.userId = user._id; // Set user ID in session on successful login
        console.log(`Login Successful for user: ${user.email}. Session userId set: ${req.session.userId}`);
        res.redirect('/home.html?login=success'); // Redirect to home page on success
    } catch (error) {
...
app.post('/register', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        console.log('Registration successful! User saved to DB:', email);
        // After successful registration, you might want to automatically log the user in
        // or redirect them to login.html. For now, we redirect to login.
...
app.get('/api/current-user-enrollments', isAuthenticated, async (req, res) => {
    try {
        ...
        if (!user) {
            console.log(`[API/enrollments] User with ID ${req.session.userId} not found in DB.`);
            req.session.destroy(); // Invalidate the stale session
...
// API Route: Get User Profile Details (Enhanced Logging)
app.get('/api/user-profile', isAuthenticated, async (req, res) => {
    console.log(`[API/user-profile] Attempting to fetch profile for userId: ${req.session.userId}`);
    try {
        const user = await User.findById(req.session.userId).select('-password');
        if (!user) {
            console.log(`[API/user-profile] User with ID ${req.session.userId} not found in DB.`);
            req.session.destroy();
            return res.status(404).json({ message: 'User not found.' });
        }
        console.log(`[API/user-profile] Found user: ${user.email}. Sending profile data.`);
        res.json(user);
    } catch (error) {
        console.error('[API/user-profile] Error fetching user profile details:', error);
...
app.get('/logout', (req, res) => {
    console.log(`[Logout] Attempting to destroy session for userId: ${req.session.userId}`);
...
app.get('/:page.html', (req, res) => {
    ...
    res.sendFile(path.join(__dirname, 'public', `${page}watch-vedio.html`), (err) => {
        if (err) {
